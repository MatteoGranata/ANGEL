      ...
      <div class="flex flex-col sm:flex-row justify-center w-full h-full">
        ...
        <ul class="grid grid-cols-2 gap-5 md:grid-cols-2 h-full w-full mt-5 sm:ml-5 sm:mt-0">
          <div class="flex flex-row justify-center h-full rounded-xl text-slate-800">
            ...
            >
              <li
                v-for="piggyBank in userProfits"
                :key="piggyBank._id"
              ...
              >
                <div class="resize-none h-fit w-full">
                  <textarea
                    ...
                    v-model="piggyBank.nameProfit"
                    name="nameProfit"
                    placeholder="name profit..."
                    rows="1"
                ...
                <div class="mt-2 flex-row flex">
                  <button
                    @click="deleteProfit(piggyBank._id)"
          ...
          <div class="flex flex-row justify-center h-full rounded-xl text-slate-800">
            ...
            >
              <li
                v-for="piggyBank in userExpenses"
                :key="piggyBank._id"
                class="flex flex-col bg-ghost rounded-xl w-full h-fit p-1 shadow-lg"
              >
                <div class="resize-none h-fit w-full">
                  <textarea
                    @keyup="autoUpdateExpense(piggyBank._id)"
                    v-model="piggyBank.nameExpense"
                    name="nameExpense"
                ...
                <div class="mt-2 flex flex-row">
                  <button
                    @click="deleteExpense(piggyBank._id)"
                    class="rounded-md px-2 w-8 h-8 text-lg mx-2 shadow-3xl ring-2 ring-inset ring-slate-100/50 hover:bg-snow"
                  >
                    <img src="../assets/icon/trashcan.png" alt="delete" />
                  </button>
  ...
  methods: {
    async fetchBalance() {
      try {
        ...
        const response = await axios.get(
          `https://angel-fq3c.onrender.com/piggybank/${projectID}/balance`,
          {
    ...
    async createProfit() {
      try {
        ...
        const response = await axios.post(
          'https://angel-fq3c.onrender.com/piggybank/profit',
          { nameProfit: this.nameProfit, projectId: projectID },
          {
            headers: { Authorization: `bearer ${token}` }
          }
        )
    ...
    async createExpense() {
      try {
        ...
        const response = await axios.post(
          'https://angel-fq3c.onrender.com/piggybank/expense',
          { nameExpense: this.nameExpense, projectId: projectID },
          {
            headers: { Authorization: `bearer ${token}` }
          }
        )
    ...
    async updateProfit(profitId) {
      try {
        ...
        const response = await axios.patch(
          `https://angel-fq3c.onrender.com/piggybank/profit/${profitId}`,
          {
    ...
    async updateExpense(expenseId) {
      try {
        ...
        const response = await axios.patch(
          `https://angel-fq3c.onrender.com/piggybank/expense/${expenseId}`,
          {
