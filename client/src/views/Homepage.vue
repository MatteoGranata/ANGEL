                ...
                <div class="flex justify-center flex-col items-center mx-2 ">
                    <li v-for="project in userProjects" :key="project._id"
                        ...
                        <input @click="goToProject(project._id)" v-model="project.name" name="projectName"
                            class="bg-ghost drop-shadow-3xl truncate overflow-hidden rounded-md w-full h-7 resize-none pl-1 cursor-alias text-slate-800 font-bold shadow-lg outline-none text-lg"></input>
                ...
                <div class="mb-3 z-50">
                    <div class="mx-4 my-1.5 flex">
                        <div class="w-fit justify-center flex flex-col items-center" @mouseover="showMessage = true"
                            ...
                            <p class="text-4xl cursor-help text-slate-600">
                                &#128712;
                            </p>
                        </div>
                    </div>
                ...
                <ul
                    ...
                    <li v-for="password in userPasswordsPinned" :key="password._id" v-show="password.pin !== false"
                        ...
                        <div class="resize-none h-fit w-full">
                            <textarea @keyup="autoPasswordUpdate(password._id)" v-model="password.passwordContent"
                                name="password" rows="1"
                        ...
                        <div class="flex rounded-md w-full bg-ghost truncate">
                            ...
                            <input @keyup="autoPasswordUpdate(password._id)" v-else type="password"
                                v-model="password.secret" name="hiddenPassword"
                                class="block flex-1 w-60 bg-transparent rounded-xl py-1.5 pl-2" />
    ...
    methods: {
        ...
        async updatePassword(passwordId) {
            try {
                ...
                const passwordToUpdate = this.userPasswordsPinned.find(password => password._id === passwordId);
                if (!passwordToUpdate) {
