      ...
      <div class="flex items-center sm:items-start flex-col h-[95vh] w-full bg-ghost rounded-xl p-5">
        <div class="absolute right-10 top-6 w-fit justify-end flex place-items-end">
          <router-link to="/homepage">
            <img width="45" height="45" src="https://img.icons8.com/ios-filled/100/1E293B/long-arrow-left.png"
              alt="Back home" class="w-8 lg:w-10" />
          </router-link>
        </div>
        <img v-if="user.avatar" :src="`${baseUrl}${user.avatar}`" alt="Immagine Profilo"
        ...
        <div class="flex flex-col relative -top-10 left-14 lg:absolute sm:left-32 lg:left-48 lg:top-60 items-start justify-between ">
          <label for="file"
          ...
          <p v-show="errorimage" class="flex text-sm lg:text-md absolute lg:static top-10 -left-36 w-60 lg:w-full items-center bg-red-500 text-slate-800 px-2 py-1 lg:px-4 lg:py-3 z-20 rounded font-medium">
            {{ errorimage }}
          </p>
        </div>
        ...
        <div class="lg:ml-4 mt-10 w-full lg:w-fit">
          <div class="bg-ghost rounded-xl px-4 py-1 drop-shadow-3xl">
            <p class="text-sm w-fit rounded-lg text-slate-800 text-start relative top-1.5">Username:</p>
          ...
          <div class="bg-ghost rounded-xl px-4 py-1 mt-5 drop-shadow-3xl">
            ...
            <p class="text-lg lg:text-2xl font-medium text-start w-fit h-fit text-slate-900">{{ user.email }}</p>
          </div>
        </div>
      </div>
    </div>
