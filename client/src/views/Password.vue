                ...
                <ul
                    ...
                    <li v-for="password in userPasswords" :key="password._id"
                        ...
                        <div class="resize-none h-fit w-full">
                            <textarea @keyup="autoUpdate(password._id)" v-model="password.passwordContent"
                                name="password" placeholder="Name Password..." rows="1"
                        ...
                        <div class="flex rounded-md w-full bg-ghost text-slate-800">
                            <input @keyup="autoUpdate(password._id)" v-if="password.show" type="text"
                                v-model="password.secret" name="passwordView"
                                class="block flex-1 w-60 bg-transparent rounded-xl py-1.5 pl-2 focus:ring-0 sm:leading-6 outline-none" />
                            <input @keyup="autoUpdate(password._id)" v-else type="password" v-model="password.secret" name="passwordHidden"
                                class="block flex-1 w-60 bg-transparent rounded-xl py-1.5 pl-2 focus:ring-0 sm:leading-6 outline-none" placeholder="Password..."/>
    ...
    methods: {
        async fetchPassword() {
            try {
                ...
                this.userPasswords = response.data.passwords.map(passwords => ({ ...passwords, show: false }));
            } catch (error) {
                console.error(error);
            }
        },
        async createPassword() {
            try {
                ...
                const response = await axios.post('https://pippo-bn7v.onrender.com/password', { passwordContent: this.passwordContent, secret: this.secret, projectId: projectID }, {
                    headers: { Authorization: `bearer ${token}` },
                });
