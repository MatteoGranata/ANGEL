          ...
          <form class="space-y-4 md:space-y-6" @submit.prevent="login">
            <div>
              ...
              <input type="username" name="username" id="username" v-model="username"
                ...
                placeholder="example434" required="true">
            </div>
            <div>
            ...
            <p v-if="errorLogin" class="flex items-center bg-red-500 text-slate-800 px-4 py-3 z-20 rounded font-bold">
              {{ '!! ' + errorLogin + ' !!' }}
            </p>
            <p class="text-md font-light text-slate-800">
              Don’t have an account yet?
              <router-link to="/register" class="font-bold text-slate-800 hover:underline">Sign up</router-link>
            </p>
          </form>
        </div>
      </div>
